...
function c5843.initial_effect(c)
	...
	e1:SetCondition(c5843.fscon)
	e1:SetOperation(c5843.fsop)
	c:RegisterEffect(e1)
	--summon success
	local e2=Effect.CreateEffect(c)
	e2:SetType(EFFECT_TYPE_SINGLE)
	e2:SetCode(EFFECT_MATERIAL_CHECK)
	e2:SetValue(c5843.matcheck)
	c:RegisterEffect(e2)
	--Double attack
	...
	e4:SetDescription(aux.Stringid(5843,0))
	e4:SetCategory(CATEGORY_TOHAND)
	e4:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e4:SetCode(EVENT_TO_GRAVE)
	e4:SetProperty(EFFECT_FLAG_DAMAGE_STEP+EFFECT_FLAG_DELAY)
	e4:SetCondition(c5843.thcon)
	e4:SetTarget(c5843.thtg)
	e4:SetOperation(c5843.thop)
	c:RegisterEffect(e4)
end

function c5843.fscon(e,g,gc,chkf)
	if g==nil then 
		return false 
	end
	if gc then 
...
function c5843.fsop(e,tp,eg,ep,ev,re,r,rp,gc,chkf)
	if gc then 
		...
		mg:AddCard(gc)
		if g1:GetCount()>0 and Duel.SelectYesNo(tp,aux.Stringid(5843,1)) then
...
function c5843.thcon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	return c:IsPreviousLocation(LOCATION_MZONE) and bit.band(c:GetSummonType(),SUMMON_TYPE_FUSION)==SUMMON_TYPE_FUSION 
end
function c5843.filter(c)
	return c:IsSetCard(0x9b) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand()
end
function c5843.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c5843.filter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c5843.filter,tp,LOCATION_GRAVE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectTarget(tp,c5843.filter,tp,LOCATION_GRAVE,0,1,1,nil)
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,g,1,0,0)
end
function c5843.thop(e,tp,eg,ep,ev,re,r,rp)
