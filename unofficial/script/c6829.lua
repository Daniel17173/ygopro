...
function c6829.initial_effect(c)
	...
	e2:SetDescription(aux.Stringid(6829,0))
	e2:SetCategory(CATEGORY_DISABLE)
	e2:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
	e2:SetProperty(EFFECT_FLAG_NO_TURN_RESET)
	e2:SetCode(EVENT_CHAINING)
	e2:SetRange(LOCATION_PZONE)
	e2:SetCondition(c6829.negcon)
	e2:SetTarget(c6829.negtg)
	e2:SetOperation(c6829.negop)
	c:RegisterEffect(e2)
	local e3=Effect.CreateEffect(c)
	e3:SetCategory(CATEGORY_TODECK)
	e3:SetDescription(aux.Stringid(6829,0))
	e3:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e3:SetCode(EVENT_DAMAGE_STEP_END)
	e3:SetRange(LOCATION_MZONE)
	e3:SetCondition(c6829.effcon)
	e3:SetTarget(c6829.efftg)
	e3:SetOperation(c6829.effop)
	c:RegisterEffect(e3)
end
...
function c6829.negop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.SelectYesNo(tp,aux.Stringid(6829,3)) then
		e:GetHandler():RegisterFlagEffect(6829,RESET_EVENT+0x1fe0000+RESET_PHASE+PHASE_END,0,1)
		Duel.NegateEffect(ev)
		Duel.Destroy(e:GetHandler(),REASON_EFFECT)
	else end
end
function c6829.effcon(e,tp,eg,ep,ev,re,r,rp)
...
function c6829.efftg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if Duel.GetFieldGroupCount(tp,0,LOCATION_MZONE)==0 then
		opt=Duel.SelectOption(tp,aux.Stringid(6829,1))
	else
		opt=Duel.SelectOption(tp,aux.Stringid(6829,0),aux.Stringid(6829,1))
	end
	e:SetLabel(opt)
	if opt==0 then
...
function c6829.effop(e,tp,eg,ep,ev,re,r,rp)
