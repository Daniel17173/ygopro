...
function c6810.initial_effect(c)
	...
	e6:SetCondition(c6810.otcon)
	e6:SetOperation(c6810.otop)
...
function c6810.splimit(e,c,tp,sumtp,sumpos)
	return not c:IsSetCard(0x9a) and bit.band(sumtp,SUMMON_TYPE_PENDULUM)==SUMMON_TYPE_PENDULUM
end
function c6810.filter(c)
	return c:IsSetCard(0x9a) and c:IsFaceup() and c:GetLevel()>0
end
function c6810.lvtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and c6810.filter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c6810.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TARGET)
	Duel.SelectTarget(tp,c6810.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,1,nil)
end
