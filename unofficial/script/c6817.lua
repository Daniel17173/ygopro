...
function c6817.initial_effect(c)
	...
	e1:SetCondition(c6817.otcon)
	e1:SetOperation(c6817.otop)
	...
	e2:SetCountLimit(1,6817)
	e2:SetCondition(c6817.spcon)
	e2:SetTarget(c6817.sptg)
	e2:SetOperation(c6817.spop)
	c:RegisterEffect(e2)
end
...
function c6817.filter(c,e,tp)
	return c:IsSetCard(0x2b) and c:GetCode()~=6817 and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end

function c6817.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c6817.filter(chkc,e,tp) end
