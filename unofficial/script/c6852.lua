...
function c6852.initial_effect(c)
	...
	e2:SetCountLimit(1)
	e2:SetCondition(c6852.spcon)
	e2:SetTarget(c6852.sptg)
	e2:SetOperation(c6852.spop)
	c:RegisterEffect(e2)
	--Equip
	...
	e3:SetCondition(c6852.condition)
...
function c6852.filter(c,e,tp)
	return c:IsCode(78193831) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c6852.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and c6852.filter(chkc,e,tp) end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingTarget(c6852.filter,tp,LOCATION_GRAVE,0,1,nil,e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectTarget(tp,c6852.filter,tp,LOCATION_GRAVE,0,1,1,nil,e,tp)
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,g,1,0,0)
end
function c6852.spop(e,tp,eg,ep,ev,re,r,rp)
...
function c6852.filter2(c)
	return c:IsSetCard(0x1e7) and c:IsType(TYPE_MONSTER)
end
function c6852.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_SZONE)>0
		and Duel.IsExistingTarget(c6852.cfilter,tp,LOCATION_MZONE,0,1,nil)
		and Duel.IsExistingMatchingCard(c6852.filter2,tp,LOCATION_GRAVE,0,1,nil) end
