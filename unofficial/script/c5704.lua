...
function c5704.initial_effect(c)
	...
	e3:SetTarget(c5704.target)
	e3:SetOperation(c5704.operation)
	c:RegisterEffect(e3)
	local e4=e3:Clone()
	e4:SetCode(EVENT_SPSUMMON_SUCCESS)
	c:RegisterEffect(e4)
end
function c5704.spcon(e,c)
	if c==nil then return true end
	return Duel.GetLocationCount(c:GetControler(),LOCATION_MZONE)>0 and
		Duel.IsExistingMatchingCard(c5704.filter,c:GetControler(),LOCATION_MZONE,0,1,nil)
end
...
function c5704.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(c5704.lvfilter,tp,LOCATION_MZONE+LOCATION_GRAVE,0,1,nil,e:GetHandler():GetLevel()) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
	Duel.SelectTarget(tp,c5704.lvfilter,tp,LOCATION_MZONE+LOCATION_GRAVE,0,1,1,nil,e:GetHandler():GetLevel())
end
function c5704.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	e1:SetTarget(c5704.sumlimit)
	Duel.RegisterEffect(e1,tp)
end
function c5704.sumlimit(e,c,sump,sumtype,sumpos,targetp,se)
