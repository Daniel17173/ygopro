...
function c6702.initial_effect(c)
	...
	e1:SetCost(c6702.cost)
	e1:SetTarget(c6702.target)
...
function c6702.cost(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c6702.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c6702.filter(chkc,e,tp) end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingTarget(c6702.filter,tp,LOCATION_GRAVE,0,1,e:GetHandler(),e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectTarget(tp,c6702.filter,tp,LOCATION_GRAVE,0,1,1,e:GetHandler(),e,tp)
