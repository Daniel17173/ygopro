--召喚神エクゾディア
function c5401.initial_effect(c)
	c:EnableReviveLimit()
	...
	e2:SetCondition(c5401.spcon1)
	e2:SetOperation(c5401.spop1)
	c:RegisterEffect(e2)
	...
	e5:SetTarget(c5401.target2)
	e5:SetOperation(c5401.operation2)
	c:RegisterEffect(e5)
...
function c5401.exodfilter(c)
	return c:IsCode(8124921) or c:IsCode(44519536) or c:IsCode(70903634) or c:IsCode(7902349) or c:IsCode(33396948)
end
function c5401.spcon1(e,c)
	if c==nil then return true end
	return Duel.GetLocationCount(c:GetControler(),LOCATION_MZONE)>-1
		and Duel.CheckReleaseGroup(c:GetControler(),c5401.exodfilter,1,nil)
end
function c5401.spop1(e,tp,eg,ep,ev,re,r,rp,c)
	if c==nil then return true end
	local g=Duel.SelectReleaseGroup(c:GetControler(),c5401.exodfilter,1,1,nil)
	Duel.Release(g,REASON_COST)
end

function c5401.efilter(e,te)
	return te:GetOwner()~=e:GetOwner()
end
function c5401.atkval(e,c)
	return Duel.GetMatchingGroupCount(c5401.exodfilter,c:GetControler(),LOCATION_GRAVE,0,nil)*1000
end
...
function c5401.operation2(e,tp,eg,ep,ev,re,r,rp)
...
function c5401.spop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_CONFIRM)
	local cg=Duel.SelectMatchingCard(tp,c5401.exodfilter,tp,LOCATION_HAND,0,1,99,nil)
