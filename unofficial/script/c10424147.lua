...
function c10424147.initial_effect(c)
	...
	e2:SetCost(c10424147.tcost)
	e2:SetTarget(c10424147.ttarget)
	e2:SetOperation(c10424147.tactivate)
	c:RegisterEffect(e2)
	...
	e3:SetCost(c10424147.cost)
	e3:SetTarget(c10424147.target)
	e3:SetOperation(c10424147.activate)
	c:RegisterEffect(e3)
end
function c10424147.tcost(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c10424147.filter1(c,e,tp)
	...
	return c:IsFaceup() and c:IsSetCard(0x10d5)
		and Duel.IsExistingMatchingCard(c10424147.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,c,att)
end
...
function c10424147.ttarget(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c10424147.tactivate(e,tp,eg,ep,ev,re,r,rp)
...
function c10424147.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():IsAbleToGraveAsCost() end
	Duel.SendtoGrave(e:GetHandler(),REASON_COST)
end
function c10424147.filter(c,e,tp)
...
function c10424147.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local gf=Duel.GetMatchingGroup(c10424147.filter,tp,LOCATION_MZONE,0,nil,e,tp,e:GetHandler())
	local ct=g:GetClassCount(Card.GetCode)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE+LOCATION_MZONE) and chkc:IsControler(tp) and c10424147.filter(chkc,e,tp) end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>-1 and ct>2
		and Duel.IsExistingTarget(c10424147.filter,tp,LOCATION_GRAVE+LOCATION_MZONE,0,3,nil,e,tp)
		and Duel.IsExistingTarget(c10424147.filter,tp,LOCATION_GRAVE+LOCATION_MZONE,0,3,nil,e,tp) end
...
function c10424147.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,c10424147.rfilter,tp,LOCATION_EXTRA,0,1,1,nil,e,tp,mg0)
	local sc=g:GetFirst()
	if sc then
