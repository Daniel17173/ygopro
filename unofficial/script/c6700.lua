...
function c6700.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		local g=Duel.GetMatchingGroup(c6700.tgfilter,tp,LOCATION_HAND+LOCATION_DECK,0,nil)
		return g:GetClassCount(Card.GetCode)>1
			and Duel.IsExistingMatchingCard(c6700.spfilter,tp,LOCATION_DECK,0,1,nil,e,tp)
	end
...
function c6700.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SendtoGrave(tg1,REASON_EFFECT)~=0 then
		local g=Duel.SelectMatchingCard(tp,c6700.spfilter,tp,LOCATION_DECK,0,1,1,nil,e,tp)
		local tc=g:GetFirst()
		if tc then
			...
			e1:SetCountLimit(1)
			e1:SetOperation(c6700.thop)
			e1:SetReset(RESET_EVENT+0x1fe0000)
			tc:RegisterEffect(e1,true)
		end
	end
end
function c6700.thop(e,tp,eg,ep,ev,re,r,rp)
