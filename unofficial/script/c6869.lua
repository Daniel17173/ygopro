...
function c6869.initial_effect(c)
	...
	e2:SetTarget(c6869.sumlimit)
	c:RegisterEffect(e2)
	local e3=Effect.CreateEffect(c)
	e3:SetType(EFFECT_TYPE_CONTINUOUS+EFFECT_TYPE_FIELD)
	e3:SetRange(LOCATION_SZONE)
	e3:SetCode(EVENT_LEAVE_FIELD)
	e3:SetCondition(c6869.descon2)
	e3:SetOperation(c6869.desop2)
	c:RegisterEffect(e3)
end
...
function c6869.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chkc then return chkc:IsControler(tp) and c6869.filter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c6869.filter,tp,LOCATION_MZONE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TARGET)
	local g=Duel.SelectTarget(tp,c6869.filter,tp,LOCATION_MZONE,0,1,1,nil)
...
function c6869.descon2(e,tp,eg,ep,ev,re,r,rp)
	local tc=e:GetHandler():GetFirstCardTarget()
	return tc and eg:IsContains(tc)
end
function c6869.desop2(e,tp,eg,ep,ev,re,r,rp)
