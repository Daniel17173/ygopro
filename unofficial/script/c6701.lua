...
function c6701.tdtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		local g=Duel.GetMatchingGroup(c6701.tgfilter,tp,LOCATION_HAND+LOCATION_DECK,0,nil)
...
function c6701.thcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c6701.cfilter,tp,LOCATION_HAND,0,1,nil) end
	Duel.DiscardHand(tp,c6701.cfilter,1,1,REASON_COST+REASON_DISCARD)
end
...
function c6701.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local sg=Duel.SelectTarget(tp,c6701.thfilter,tp,LOCATION_GRAVE,0,1,1,nil)
