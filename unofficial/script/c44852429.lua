...
function c44852429.initial_effect(c)
	...
	e3:SetCondition(c44852429.retcon)
	e3:SetTarget(c44852429.rettg)
	e3:SetOperation(c44852429.retop)
	c:RegisterEffect(e3)
end
...
function c44852429.retcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsReason(REASON_DESTROY)
end
function c44852429.filter(c)
	return c:IsFaceup() and c:IsSetCard(0xae)
end
function c44852429.rettg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c44852429.filter,tp,LOCATION_SZONE,0,1,nil) end
	Duel.SetTargetPlayer(tp)
	local rec=Duel.GetMatchingGroupCount(c44852429.filter,tp,LOCATION_SZONE,0,nil)*1000
	Duel.SetTargetParam(rec)
	Duel.SetOperationInfo(0,CATEGORY_RECOVER,nil,0,tp,rec)
end
function c44852429.retop(e,tp,eg,ep,ev,re,r,rp)
	local rec=Duel.GetMatchingGroupCount(c44852429.filter,tp,LOCATION_SZONE,0,nil)*1000
